...
class News {
    ...
    public function renderNewsTable(array $new_rows) {
        ...
        foreach ($new_rows as $new_row) {
            $out.= '<tr>';
                ...
                $out.= '<td>'.date('Y-m-d', $new_row['date_publish_end']).'</td>';
                $out.= '<td>
                            <a href = "reklamniy_block.php?action=edit&id='.$new_row['id'].'">Редактировть</a>|
                            <a href = "reklamniy_block.php?action=delete&id='.$new_row['id'].'">Удалить</a></td>';
            $out.= '</tr>';
        }
        $out.= '</table>';
        return $out;
    ...
    public function renderNewEditForm($row_new = null) {
        ...
        $out = '';
        $out .='<form name="addNewForm" class="" action="reklamniy_block.php?action='.$form_action.'" method="post">';
            $out .='<table><tbody>';
            $out .='<input type="hidden" name="id" value="'.$id.'">';
            $out .='<tr><td>Заголовок:</td><td><input type="text" name="title" value="'.$title.'"></td></tr>';
            $out .='<tr><td>Краткий текст:</td><td><input type="text" name="short_text" value="'.$short_text.'"</td></tr>';
    ...
    public function renderAddNewButton() {
        $out = '<a href = "reklamniy_block.php?action=add">Добавить</a>';
        return $out;
    }
